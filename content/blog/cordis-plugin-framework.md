...
For a developer evaluating agent infrastructure, the Cordis-based design changes what "open source" means for a harness. An open license makes code inspectable; a plugin kernel makes it _transformable_. If you want a different sandbox, a custom tool, a different model backend, or a UI that matches your product, you mount a plugin instead of maintaining a fork. The plugin ecosystem becomes the moat — and DeepSeek has signaled that intent by adding the dsh-plugin topic on GitHub for discoverability and standing up a Discord community around the harness.
