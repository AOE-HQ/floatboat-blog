---
title: "How to Automate Weekly TikTok Shop Research with FastMoss and Floatboat"
description: "See how Floatboat operates FastMoss in its built-in browser, saves live evidence, compares history, writes and checks a management brief, delivers it, and returns through FloatSchedule."
...
**TL;DR**
  * **FastMoss supplies the live TikTok Shop signal:** markets, categories, products, creators, videos, ads, livestreams, shops, and consumer feedback.

  * **Floatboat operates the software itself:** it navigates the authorized website, changes filters, opens deeper modules, reads dynamically rendered pages, and captures evidence.

  * **The file system becomes operational memory:** screenshots, source notes, CSVs, comparisons, reports, decisions, logs, and delivery drafts remain organized on disk.

  * **The model becomes an analyst with continuity:** it compares the new run with prior snapshots and produces a recommendation, confidence level, risks, and next actions.

  * **Floatboat checks its own deliverable:** it opens the report in the built-in browser, verifies numbers and sources, checks images, links, layout, and console output, then revises the underlying file if needed.

  * **FloatSchedule brings the whole desk back:** the next run starts with the same project, rules, history, destinations, and delivery policy—not a blank chat.

## How to Automate Weekly TikTok Shop Research with FastMoss and Floatboat

 _Every Monday at 8, Floatboat runs the desk. Give it one category, one decision policy, and one FloatSchedule, and it can open the live website, investigate the signal, preserve the evidence, write and check the management brief, and leave the operation ready for the next run._
...
In the browser, it can:
  ...
  1. open the saved FastMoss ranking;

  2. confirm the country, category, date range, and ranking type;

  3. scan for meaningful changes instead of merely collecting rows;

  4. open candidate products;

  5. continue through creator, video, paid-media, livestream, VOC, and comment modules;

  6. preserve the rendered evidence with source URL and access time.
...
FastMoss estimated:
  ...
  * **+77.13% week-over-week unit growth** ;

  * a **$17.00** price and **15%** creator commission.
...
The 28-day distribution view showed:
  ...
  * **86% of units driven by creators** ;

  * **80% of units attributed to video** ;

  * approximately **79% attributed to paid video traffic** ;

  * an estimated ROAS near **4.49** ;

  * thousands of related ad creatives.
...
The DR.DENT cycle left **20 auditable files** in one dated project run, including:
  ...
  * seven live source screenshots;

  * structured live-evidence records;

  * a historical baseline and metric-level comparison;

  * a Markdown management brief;

  * a rendered HTML management brief;

  * the decision recommendation and confidence;

  * a run manifest and execution log;

  * a self-review record;

  * a management email draft;

  * a checksum inventory of the final package.
...
It revisits the ranking, captures the new date, reopens DR.DENT and the validation queue, and compares the new values with the prior evidence. The report can focus on deltas:
  ...
  * Did weekly rank, units, or GMV materially change?

  * Is paid-video dependency rising or falling?

  * Did the creator network broaden or concentrate?

  * Are ratings, comments, or claim risks deteriorating?

  * Did the product cross a decision threshold?
...
The FastMoss case is one vertical example of a broader runtime architecture:
  ...
  * **The built-in browser is action.** Floatboat can work inside the same authorized web software a human uses, including dynamic interfaces that have no convenient integration yet.

  * **The file system is continuity.** Sources, intermediate work, final outputs, decisions, and failures survive the conversation and remain inspectable.

  * **Models and tools are the analysis team.** They compare states, detect contradictions, calculate changes, draft recommendations, and transform evidence into different deliverables.

  * **FloatSchedule is time.** It starts the work with context attached, allowing the same Agent operation to return on a business rhythm.

  * **Email, calendar, and connected services are reach.** The work can enter management, collaboration, and follow-up systems instead of ending in a chat response.

  * **Workflows and Combo Skills are institutional memory.** Once a reliable operating pattern is learned, it can be packaged, reused, improved, and distributed.
...
The same pattern can support:
  ...
  * a multi-market TikTok Shop category radar;

  * a daily competitor and shop movement monitor;

  * creator discovery with evidence-backed outreach queues;

  * paid-content and creative-pattern intelligence;

  * product validation programs with dated decision gates;

  * weekly client intelligence reports for agencies;

  * a management exception desk that only escalates meaningful changes;

  * bilingual partner reports generated from the same evidence package.
