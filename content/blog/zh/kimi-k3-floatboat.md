...
能用。Kimi K3 已通过 Floatboat 的模型选择器提供，用户无需配置单独的 Kimi API Key。

### Kimi K3 是开源的吗？

Kimi 称 K3 为开放模型，并表示完整模型权重将在 2026 年 7 月 27 日前发布。截至本文写作时，权重尚未公开。一旦发布，K3 将成为 3T 参数级别里第一款开源权重的模型。

### Kimi K3 有百万 token 上下文窗口吗？

有。Kimi 官方文档明确给出 100 万 token 上下文窗口。Kimi K3 发布初期默认以最大推理强度运行，更低与更高的强度模式计划在后续更新中提供。
...
### 在 Floatboat 里用 K3 需要 Kimi API Key 吗？

不需要。Floatboat 提供内置模型访问。在 Floatboat 工作区里使用 Kimi K3 无需单独的供应商 API Key。
