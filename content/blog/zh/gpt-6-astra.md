...
### 为什么 Astra 的 ARC-AGI-3 分数差别这么大？

因为两个数字量的是不同的东西。99.9% 出自 OpenAI 自带的评测实现——它在轮次间保留推理状态、压缩长上下文，是 OpenAI 交付的“产品”；62.7% 出自 ARC Prize 的标准化评测，隔离的是模型本身能力，同样处于当前最高水准。两数都成立，都不宜单独引用。

### 为什么我用不到 Astra 的全部网络安全能力？

因为 OpenAI 认定 Astra 是其框架下第一个达到“严重（Critical）”级网络能力的模型：能发现未知漏洞，在加固系统上搭建利用链。因此最先进的网络工作流只开放给 Daybreak Blue 审核过的防御方，数周内逐步放宽；标准版拒绝高危漏洞利用，护栏更严——对正当工作反而更安全。

### Astra 真的算得上最智能的模型吗？

并非全面领先——为一个标题买单前值得知道。Astra 真正强在电脑操作（OSWorld 2.0 上 72.6%、单任务省约 47% 时间）、单任务效率与专业科学与网络安全；但独立 Artificial Analysis 榜上它仅 61.2 分，低于 Claude Fable 5.1（65.7）与 Claude Opus 5（63.1）。“世界最智能模型”更像宣传，而非全面领先的战绩。

### GPT-6 Astra 怎么收费？

标准 API 价为每百万输入 token 10 美元、每百万输出 50 美元——是 GPT-5.6 Sol 促销价的 2.5 倍，与 Claude Fable 5.1 持平；缓存写入与超长提示另有溢价档。OpenAI 主张按“每完成任务的成本”算：Astra 在许多任务上 token 与重试都更少。这笔账值得你用自己的负载验证后再买单。

### 该从 GPT-5.6 Sol 换到 GPT-6 Astra 吗？

先等你能用自己的任务实测，账本划算再换。Astra 最强的是长周期电脑操作（单任务省约 47% 时间）与 105 万 token 级的整仓分析；GPT-5.6 Sol 做常规 agent 工作仍是更便宜可靠的主力，高并发下 token 省钱往往盖过效率收益。等 API 放开后，两模型都跑你的负载，再对比每任务成本与介入率。

### 什么时候才能在 API 或 Floatboat 里用上 Astra？

还要再等等。Astra 于 9 月 3 日发布，先供 Daybreak 组织使用；ChatGPT 各套餐、API 与 AWS 接入据称“未来几天内”跟上。Floatboat 今天还没有它——内置名单仍是 GPT-5 家族与 DeepSeek、MiniMax、GLM、Kimi、Claude、Gemini，集成工作已在推进；在真正可用之前，我们不会假装能用。
