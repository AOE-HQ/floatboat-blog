...
要创建 Custom GPT，需要——Plus（每月 20 美元）、Team、Business 或 Enterprise 之一。免费档用户可以使用别人发布的 GPT，但不能自己构建。

### ChatGPT 能在我离线时运行任务吗？

Custom GPT 只在对话进行中工作。关掉对话，它就停了。Agent Mode 能让部分任务在后台运行；Business/Enterprise 的 Workspace Agents 能独立地在云端运行。

### 可复用的指令该怎么存？

把你的 GPT 指令放在一个单独的文本文件里，按日期做版本管理。每次更新都保存上一版。我已经有两次因为一个"改进"弄坏了东西而回滚。构建器也有版本历史，但我喜欢手里有份自己的备份。

### 哪些权限不该给它？

不理解会发出什么数据，就别连接外部 API。处理机密信息的 GPT 不要开启网页浏览。团队套餐里，别跳过管理员控制——限制 action 域名，并每季度复核一次访问权限。

### 怎么知道 Agent 够不够可靠？

拿你真实工作流里的 10 个现实场景跑一遍。如果 10 个里你要纠正超过 2 个，说明指令还需要打磨。少于 1/10——你就有一件值得留下的东西了。最大的错误是：指令写一次就再也不更新。把你的 GPT 当软件对待。
