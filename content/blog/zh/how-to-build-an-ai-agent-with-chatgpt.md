...
不需要。作者只用 Custom GPT 构建器、几个上传的文件和一段重写了三次的指令，就做出了一个能自己跑的内容调研 Agent——没有代码、没有 API。难点不在技术，而在把任务定义精确、写出清晰的分步指令，再用真实的边角案例去测试。只要能说清你每周重复的那件事，你就能为它搭一个 GPT。

### Custom GPT 到底能做什么？

Custom GPT 能执行你定义的指令、引用上传的文件、浏览网页、运行代码做数据分析、生成图片——可以把它想成"背景交代充分、每次都照同一本 playbook 执行的助理"。它不能做的，是在外部网站上采取动作、替你发邮件或运行后台任务；那些属于独立的 Agent Mode，或 Business/Enterprise 套餐的 Workspace Agents。

### 构建 Custom GPT 需要付费套餐吗？

需要。创建 Custom GPT 至少要有 Plus（每月 20 美元）、Team、Business 或 Enterprise 套餐之一；免费档用户可以用别人发布的 GPT，但不能自己构建。只有任务确实可重复、定义清晰时才值得付费——Custom GPT 的甜区，正是那些你本来会每次从零重新提示的活。

### 它能在我离线时运行任务吗？

不能。Custom GPT 只在对话进行中工作——关掉对话它就停了，而且不会跨会话保留记忆。需要后台或自主运行时，ChatGPT 的 Agent Mode 能让部分任务在后台跑；Business/Enterprise 的 Workspace Agents 则能在云端独立运行，还能在 Slack 里工作。

### 哪些权限不该给它？

不理解会发出什么数据，就别通过 Actions 连接外部 API；处理机密信息的 GPT 不要开启网页浏览。团队套餐里保持管理员控制开启：限制 action 域名，并每季度复核一次访问权限。任何产出面向客户的 GPT，都要在内容发出前保留一道人工复核步骤。

### 怎么判断 Agent 够不够可靠？

拿真实工作流里的 10 个场景跑一遍：如果 10 个里要纠正超过 2 个，说明指令还需要打磨——收紧它、补上示例；少于 1/10，就有一件值得留下的东西了。最大的错误是指令写一次就再也不更新；把你的 GPT 当软件对待，持续迭代。
