...
理念相似，执行不同。第二大脑（依 [Tiago Forte 的框架](<https://www.buildingasecondbrain.com/>)）由你手动打理；**LLM 知识库**则由 AI 从原始素材编译并维护——人工打理开销更少，但同样需要持续投喂。

### 自建一个需要会写代码吗？

Karpathy 原始那套需要。社区实现越来越易用，但你会遇到需要一定技术底子的配置决策。如果你不是开发者，这条路目前仍有真实的摩擦。

### 想测试它适不适合我，最小可行版本是什么？

从比你想象的更简单开始。把一个月的研究笔记丢进一个文件夹，用带编译 prompt 的单次 Claude 或 GPT-4 会话跑一遍，看看输出有没有用。按 [Anthropic 的 prompt 文档](<https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview>)，清晰的结构化 prompt 在你需要基础设施之前就能起很大作用。如果这让你觉得有价值，再投入去建真正的流水线。

### 这不基本就是 RAG 吗？

恰恰相反。Karpathy 的做法刻意跳过了向量数据库的复杂度。wiki 就是 markdown 文件。LLM 直接在其中导航——可读、可被人审计，个人规模下完全不需要 embedding。

### 我以前试过知识库，后来放弃了。这次会不一样吗？

诚实面对你放弃的原因。如果是打理开销，AI 编译的系统能减轻一部分。如果是因为检索价值不足以支撑维护，那是匹配度问题——而**LLM 知识库**解决不了匹配度问题。
