...
在 [agentic calendar](/zh/blog/what-is-agentic-calendar) 上，「原型截止」或「设计评审」事件可以触发与构建装饰艺术小镇相同的 K3 视觉推理路径：生成或重新生成 HTML、打开它、截一帧、对照需求简报，并在你进入会议之前把备注留在事件工作区里。你不是在让模型帮你管日历，而是让日历决定昂贵的视觉模型什么时候跑。
...
关于日常会议工作中什么时候用 K3、什么时候用 K2.7 Code 的更细映射，参见 [Kimi K3 模型详解](/zh/blog/kimi-k3-open-frontier-model)。游戏演示是压力测试；日历路由才是日常习惯。
