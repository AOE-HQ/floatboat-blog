...
![how4.png](/blog/images/how-to-build-ai-agents-for-repeated-work/1778749654339-123add4e-863e-4c0d-bab5-c8798c1436bd.png)
...
### 搭一个简单 agent 需要会写代码吗？
不需要——我甚至不建议从代码入手。对多数重复任务，一个无代码自动化工具或 AI 搭建器就够了，根本不用碰终端。只有当你需要自定义逻辑、工具不支持的 API 集成、或非常特定的错误处理行为时，编码才变得必要。从最轻的路径（第 3 步）开始，撞墙了再转向代码。

### 先从哪件重复任务开始自动化？
挑那件你每周做三次以上、方式大致相同、输入也大致相同、并且能用一句话说清的工作。好的起步对象：把工单总结成一行分诊备注、起草每周状态报告、从会议逐字稿抽行动项。坏候选是：判断每次都变、输入不可预测、或者说不清「做完」长什么样。

### 我能搭的最小可用 agent 是什么？
比你想的更小。一个真正有用的最小 agent 可能是：一条提示词模板，输入一封原始支持邮件，输出一行格式一致的摘要。没有集成、没有记忆、没有工具调用，只是一个可靠的、可重复的「一输入到一输出」转换。如果它每天帮你省 10 分钟，就值得拥有。

### 每件重复工作都需要真正的 Agent 吗？
不需要——大多数重复性工作根本用不着真正的 Agent。按从最轻到最重走决策树：单次对话能跑完的，用一条结构良好的提示词；跨应用搬数据的，用 n8n、Zapier、Make 这类自动化工具；需要来回推理、带记忆与工具的，用无代码搭建器；最后才轮到编码实现的工作区 Agent。原则：提示词能干的事不搭，搭建器能处理的不写代码。

### 一个 agent 该处理几个工作流？
一个。至少一开始是一个。加范围是个很强的诱惑——「既然它已经在读邮件了，为什么不顺便查查 CRM、起草个回复呢？」——但每加一步，出错的方式就翻倍。让你的第一个 agent 只守一条工作流。等它稳定、可信了，再决定是扩展它，还是为下一件工作另建一个 agent。

### Agent 老在同一处失败，该怎么办？
先别升级模型。agent 老在同一处失败，就回到第 2 步的「输入—判断—输出」图——你多半是撞上了一个比看起来更复杂的判断点。收窄任务，而不是加能力。若迭代两轮后，输出仍让你不敢不整段重写，就停止搭建——这个任务可能依赖尚不适合编码的隐性知识。
