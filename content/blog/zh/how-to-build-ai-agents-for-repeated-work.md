...
**一个编码实现的工作区 Agent**——只有当上面几项都做不到时才走到这。这意味着要自己写真正的逻辑、管状态、处理错误。它更强大，但维护成本是真的。真走到写代码这一步，下一个岔路口就是[编排逻辑自己写多少、交给现成工作区跑多少](/zh/blog/dynamic-workflows-build-or-use-workspace)——这笔账值得在第一行代码之前算。
