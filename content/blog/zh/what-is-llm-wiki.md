...
### 建一个 LLM wiki 必须是开发者吗？

实际上，是的——Karpathy 的配置需要终端命令，以及 Claude Code、Obsidian 这类工具的熟练度。这套架构在概念上很简单，但现有实现都假设你有技术熟练度。如果你是技术人员，请关注底层原则而非具体工具：一次编译知识、多次查询。

### LLM wiki 和 RAG 是一回事吗？

不是，而且差别很重要。RAG 每次查询都从原始素材里检索；LLM wiki 则先把那些素材预编译成一份结构化、相互链接的知识库。LLM 在 wiki 里导航，而不是重新检索原始文件；知识在查询之间积累，而不是每次清零。可以这样想：RAG 负责「找」，wiki 负责「懂」。

### 对不打算搭建的人来说，最简版本是什么？

维护一份活的上下文文档——一个描述你当前项目、工作偏好与关键决策的单一文件。偶尔更新它，在重要的 AI 会话里贴进去。这是同一个原则的手动版：一次编译上下文，别再每个会话都重新解释。

### 为什么偏偏在 2026 年、偏偏是现在火起来？

时机与 Claude Code 这类 LLM 编程 Agent 的成熟同步——是它们让这个实现从「纸上谈兵」变成「真的可行」。让 AI_维护_知识库（而不只是回答关于它的问题）这个想法，只有在 AI Agent 能可靠地自主读写文件时才变得现实。而那种基础设施现在足够好了，真能跑起来。

### Floatboat 和自己建 LLM wiki 有什么不同？

自己建 LLM wiki 需要搭建、维护与技术熟练度。而朝工作区模型走的产品（比如 [Floatboat](</>)）正在尝试把基础设施层处理好，让上下文复利的收益无需那些 overhead 就能获得。底层原则相同，实现层面的要求却大不相同。我会说，这就像自己建一间厨房和租一间已经带更好食谱的厨房的区别。
