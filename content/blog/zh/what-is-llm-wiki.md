...
一个由 LLM 从你的原始素材"预编译"出来的 markdown 文件目录——含摘要、概念页、实体页与交叉引用——外加一份告诉 AI 如何组织与更新的 schema 配置文件。Karpathy 的比喻：RAG 是"饿了才做饭"，LLM wiki 是"建一座菜谱会不断改良的厨房"。原始文档保持原样不动，簿记维护交给 LLM。

### LLM wiki 和 RAG 有什么不同？

RAG 是"检索"，LLM wiki 是"积累"。RAG 每次提问都去原始文档里现翻，次次从零开始、反复重新发现；LLM wiki 则把素材预编译成结构化、互相链接的知识库，查询时直接导航其中已被综合过的内容。每加一个新来源都是"整合"而非"追加"——所以 wiki 越用越值钱。

### Karpathy 的 LLM wiki 为什么会火？

因为它点破了一个大家早就有、却说不出名字的痛点——"每次开 AI 会话都从零开始"；也因为载体不是代码，而是一份"想法文件"：把模式粘进 LLM Agent 就能让它替你搭建。那条推文有 1600 多万次浏览，后续的 gist 几天内就过了 5000 star。时点也关键：编码 Agent 的成熟，让"让 AI 自主维护文件"第一次真正可行。

### 非技术背景能搭建 LLM wiki 吗？

实际上，门槛不低——Karpathy 自己的方案默认你会用 Claude Code、Obsidian、命令行，并且愿意在东西坏掉时动手调试。架构在概念上很简单，但现有实现是给开发者与研究者准备的。非技术读者更该带走的是那个原理——"编译一次、多次查询"——而不是具体工具链。

### 不想自建的话，最简替代版是什么？

一份常青的"上下文文档"：用 500–800 字写清你当前的项目、工作偏好与标准，重要 AI 会话开头粘进去，每月更新一次。这是同一模式的纯手动版，十分钟就能搭好。中间档还有 Notion AI、ChatGPT 的 memory 这类方案——跨会话记得住些东西，结构弱但摩擦极低；工作区型工具则正试图把搭建步骤整个省掉。

### 我该不该自建一个 LLM wiki？

看情况。如果你习惯命令行、手里有一批随时间增长的明确素材（研究、文档、你深耕的领域），并且会持续喂新来源、让维护成本摊得开——那值得建。如果你的知识库还很浅（不到 30–40 个来源），或工作多变、以即兴为主——大概率不值：一份维护良好的系统提示词加几份参考文档，零基建就能覆盖大部分同样的收益。
