...
当它需要与其他 Agent 协同、跨会话维持状态、或在超出「只有我在用」的权限结构下运行时。如果你正手动把一个 GPT 的输出复制到另一个 GPT，你已经在临界点上了；一旦你自动化那次交接，不管有没有计划，你都在**开发 Agentic AI 系统**了。

### 系统维护应该由谁负责？

由既懂工作流又懂工具的那个人负责。对单人创业者来说，那就是你——这也是为什么把事情保持简单如此重要。对小团队来说，指定一个人作为系统负责人，并确保他有时间投入。维护不是一次性的搭建成本；请为每周 2–4 小时的监控、更新和修复「悄悄漂移的问题」留出预算。

### 上线前应该准备哪些文档？

至少要有：一份用大白话说明系统能做什么、不能做什么的描述；一份它所连接的每个外部工具或 API 的清单；权限边界（能访问什么、不能访问什么）；预期的故障模式与恢复步骤；以及指令变更的版本日志。我把这些都放在同一个 markdown 文件里。它不需要精致——只需要保持最新。

### 如何避免把同一条工作流重复做两遍？

把指令做成模板，并保存在平台之外。我为每条验证过的工作流都建了一个文件夹，放着验证过的提示词、工具配置和示例输出。当另一个场景需要类似系统时，我从模板改起，而不是从零开始。另外，命名要清晰：「Research Agent v3——客户简报」六个月后你还能看懂它干什么；「我的 GPT」不行。
