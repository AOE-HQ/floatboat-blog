...
不完全是，而且谁要是这么描述它，我会温和地纠正一下。Managed Agents 被设计成一个"元 harness（meta-harness）"——一套带通用接口的系统，让从 Claude Code 到任务专用 Agent 的各种 agent harness 都能跑在同一个稳定基础设施里。Agent 执行是自主的，但任务、工具和护栏由你定义。在设计层的 human oversight（人类监督）仍然不可或缺。"自主执行"指的是 Agent 跑起来不需要你盯着每一步——而不是说它在缺少人类对"该做什么"的判断下运作。

### 还需要写 prompt 吗？

需要——因为总得有人定义 Agent 该做什么、该怎么表现。形式变了——你可以用自然语言描述 Agent，或用 YAML 配置——但 Agent 行为质量，直接取决于你把任务与约束定义得怎么样。写 prompt 的技能会转化为设计 Agent 的技能。它不会消失。

### 商用安全吗？

对大多数用标准 Claude API token 访问、没有把敏感凭据挂到 Agent 上的单人创业者：在合理的范围控制下，风险是可控的。只要涉及生产凭据、客户数据或财务访问：请务必慎重。收紧权限范围，在会话日志里审查 Agent 实际做了什么（Anthropic 的控制台支持这一点），不要给 Agent 用不到的权限。[Anthropic 关于 Managed Agents 的工程博客](<https://www.anthropic.com/engineering/managed-agents>)讲清了会话与上下文如何处理——想在投入前搞懂架构可以读它。

另外建议直接查当前 Claude Platform 文档，获取"哪些在公开 beta、哪些需要单独申请访问"的最新指引——这块变化很快，未来几个月可用的能力大概率还会扩展。
