...
Codex for Chrome 随所有 ChatGPT 套餐提供，包括 Free 档（带有限的试用额度）。Claude for Chrome 需要付费套餐——最低 Pro 档（每月 20 美元），不过 Pro 档在扩展里只能用到 Haiku 4.5。

### 浏览器 Agent 能用在任何网站上吗？

大多数网站可以。反爬很重、CAPTCHA 密集或 JavaScript 界面非常动态的站点可能不行。银行与金融网站常常默认被拦截。

### 浏览器 AI Agent 在欧盟可用吗？

Claude for Chrome 全球可用。Codex for Chrome 目前在欧盟与英国不可用——OpenAI 表示即将上线。

### 我能在自己的 localhost 开发服务器上用浏览器 Agent 吗？

不能通过 Chrome 扩展。Codex 把 localhost 路由到它的应用内浏览器；Claude 做终端到浏览器的工作流需要 Claude Code 集成。

### 我的数据安全吗？

两个工具都会通过各自的 AI 服务处理页面内容。不要在含敏感凭据、受保护的健康信息或机密客户数据的页面上使用它们——除非你的组织签有专门的数据处理协议。
