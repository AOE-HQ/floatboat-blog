...
Grok 是 AI 模型（推理引擎，最新为 Grok 4.6）。Grok Bot 是 Agent 产品：一个持久、具名的队友，跑在自己的云电脑上，登录你的应用，完成多步工作。普通 Grok 回答问题；Grok Bot 干活。

### Grok Bot 怎么访问我的应用？

Bot 使用一台带浏览器、文件系统与终端的持久云 VM。有连接器/MCP 的地方就用它们；没有干净 API 的应用与网站就使用 computer use（视觉操作界面）。Bot 用你的凭证登录，会话跨任务保持。

### Grok Bot 安全吗？

只要你理解它的模型，就可以安全使用。你账号上的每个 Bot 共享一台云电脑——同样的 cookie、文件与凭证——xAI 文档明确说不要把独立的 Bot 当作安全边界。把整个 Bot 花名册当作单一信任区，敏感系统用带作用域的账号，高危动作要求批准。

### Grok Bot 多少钱？

它不单独销售。包含于 SuperGrok Heavy（约 $300/月）、Cursor Ultra（$200/月）或 Cursor Teams Premium（每席 $120/月）。没有免费档，没有独立套餐；企业访问只能进候补名单。

### Grok Bot 能跑在我自己的电脑上吗？

主要工作在厂商的云电脑上完成，与你本地机器分开。Bot 只有在你本地电脑策略下显式启用并批准该能力时，才会运行本地命令。

### Grok Bot 与开源 Agent harness 相比如何？

它们是方向相反的赌注。Grok Bot 给你一个由厂商控制电脑的托管队友——为成果优化，但集中了你的凭证、也不可移植。DeepSeek Harness 等开源 harness 把内核与插件放在你自己的基础设施上——为掌控与可移植性优化，代价是要自己运维。
