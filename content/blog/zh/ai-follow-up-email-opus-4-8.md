...
有可能，这是最需要提防的风险。我测试时，一场结构良好的 45 分钟会议的草稿基本准确；但决议不清的混乱会议，就出现过错误的行动项。模型在进步——Anthropic 报告 Claude Opus 4.8 把自身瑕疵留而不提的可能性比上代低约四倍——但发送前亲自读稿，核对有没有凭空冒出的承诺、归错人的负责人，这一步仍不可替代。

### 什么是 AI 跟进邮件？

由 AI 根据会议上下文——通常是逐字稿或你的笔记——起草的邮件。AI 提炼出讨论了什么、定了哪些决议、谁在什么时候做什么，整理成一封带行动项的简短总结供你发送。发出前你仍需自己复核编辑。一封好的跟进邮件要确认讨论内容、让下一步可见，并制造一点点推动行动的压力。

### AI 如何在会议后起草跟进？

把会议逐字稿喂给 Claude、GPT、Gemini 之类的模型，配一条指定输出格式的提示词。模型识别决议、把行动项指派给稿中提到的人，再起草邮件。我为此存了一条提示词——配好后每场会五分钟以内搞定。Fireflies、[Read.ai](<http://Read.ai>) 这类工具内置的跟进功能，还能把这一步进一步自动化。

### 发送前我必须复核哪些内容？

三件事：语气（符不符合你和对方的关系？）、事实准确性（尤其是价格、截止日期与承诺）、完整性（AI 有没有漏掉什么——包括会上没被说出口的话？）。我从不跳过读草稿。改价、调范围这类敏感承诺，我会对照自己的笔记核对——一个归错人的行动项或凭空多出的承诺，可能造成真实的伤害。

### AI 什么时候该建任务而不是发邮件？

当会议产出的行动项只涉及内部、不需要对外沟通时。如果跟进是给自己团队的，直接把条目推进任务看板（Notion、Asana 或一张简单清单），可以完全省掉邮件。客户相关的跟进我仍会写邮件，但内部会议只建任务更快。我的判断标准：对方需要看到一份总结，还是只需要知道自己负责什么？如果是后者，跳过邮件。

### 怎么让跟进工作流可复用？

存一条固定提示词，而不是每次从零写。保持同一套结构——回顾、决议、行动项、截止日期、未决问题——唯一的变量只剩逐字稿：会开完、稿贴上、草稿出来、你复核、你发送，每场会五分钟以内。对客户对账、团队同步这类周期性会议，这个模式很好扩展：收件人习惯了固定格式，你也不用每次重造一封邮件。
