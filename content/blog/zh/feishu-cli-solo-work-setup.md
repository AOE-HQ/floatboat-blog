...
### 用 CLI 和开放平台功能需要企业版飞书账号吗？

需要。你得在[飞书开发者后台](<https://open.feishu.cn/document/server-docs/api-call-guide/calling-process/get-access-token>)创建应用，而这要求账号具备足够权限。个人免费账号有限制。如果你自己运营一个团队工作区，通常具备所需权限，但投入时间搭建之前值得先核实。

### bot token 和用户访问令牌有什么区别？

bot/tenant access token 让应用以「自己」的身份行事——适合给机器人所在的聊天发消息，或访问显式分享给应用的文档。用户访问令牌则让 CLI 代表某个具体用户行事——读取私聊、个人日历或没有显式分享给应用的文档时需要它。多数真实工作流迟早都需要用户访问令牌。

### 用户访问令牌多久过期一次，会不会弄坏自动化工作流？

用户访问令牌的有效期有限，需要定期刷新。在自动化环境里，这意味着你要么实现 token 刷新机制，要么规划按固定间隔重新授权。授权时申请 `offline_access` scope 可以获得有效期更长的 refresh token，从而延长窗口——但具体规则可能变化，值得去读平台当前文档。

### 飞书更新或弃用我正在用的 API 接口时会发生什么？

平台会带着迁移指引公告弃用，但你必须自己盯着才能看到。对单人创业者来说，实际风险是你可能直到某条工作流静默失败或返回意外结果，才发现 API 变了。保持依赖更新、定期手动测试工作流，是最可靠的缓解办法。

### 没有服务器也能定时跑飞书 CLI 工作流吗？

你可以在本地用 cron（Mac/Linux）或任务计划程序（Windows）跑命令，多数场景下够用。任何需要在你机器关机时运行、或想长期稳定运行的东西，通常都需要某种常开环境——一台廉价 VPS、一台家用服务器都行。这是实打实的基础设施投入，哪怕只是很小一笔。

### 飞书 CLI 和 Lark 开放平台是同一个东西吗？

飞书和 Lark 本质上是同一个产品——飞书是中国版，Lark 是国际版。两者开放平台的 API 大体重合，但在一个环境里创建的凭据和 App ID 无法在另一个环境使用。如果你在海外用 Lark，请确认你操作的是 Lark 开发者后台，而不是飞书的。[官方 Lark MCP 文档](<https://github.com/larksuite/lark-openapi-mcp>)里有相关澄清。

### 想了解 API 的实际范围与能力边界，最好从哪里开始？

[飞书开放平台开发者文档](<https://open.feishu.cn/document/ukTMukTMukTM/uITNz4iM1MjLyUzM>)确实全面且维护良好。建议先从流程概览与鉴权模型入手，再看具体 API 接口——理解了 access token 与权限怎么工作，当个别 API 调用行为不符预期时，你会少困惑很多。
