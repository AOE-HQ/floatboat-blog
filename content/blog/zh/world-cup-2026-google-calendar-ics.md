...
### 导入一个 ICS 文件会一次加进全部 104 场比赛吗？

会。一份世界杯 ICS 文件可以把 104 场比赛全部做成独立日历事件。只要文件完整有效，导入一个文件就能一次性加入小组赛、淘汰赛和决赛。

### 比赛时间会显示成我的本地时间吗？

通常可以。只要 ICS 文件包含时区元数据，你的日历应用就会自动把开球时间换算成你的本地时间。如果不确定，先拿一个导入事件和官方比赛时间核对，再放心使用整份日历。

### 导入 ICS 文件后，FIFA 改了比赛时间怎么办？

静态 ICS 导入不会自动更新。你需要删除旧事件，再重新导入一份更新过的文件。日历订阅——包括基于 URL 的数据源和 FloatCup——设计上就会在源数据变化时刷新。

### 能把赛程导进多个日历应用吗？

能。同一份 ICS 文件可以导入 Google Calendar、Outlook 和 Apple 日历。如果你用了 Google 或 iCloud 这类同步平台，通常在账户层面导入一次，就足以让事件出现在多个设备上。

### 赛事结束后怎么移除世界杯事件？

如果你导入到了专用日历，一步就能删掉那层日历。在 Google Calendar 中，打开"World Cup 2026"日历的设置并移除它。如果你把事件导进了主日历，就得批量删除或一个一个删。

### ICS 订阅比下载 ICS 文件更好吗？

如果提供方持续更新数据源，ICS 订阅更好。下载文件更简单、还能留一份本地副本，但导入后就是静态的。对开球时间可能变动的大赛来说，订阅更有用。
