...
### 这和 Fireflies、Otter 之类的 AI 记录工具有什么区别？

AI 记录工具捕捉会议期间发生的事——转写、摘要、待办提取。它们运行在会议内部。AI 会前准备运行在会议之前：收集上下文、调取文档、生成简报。两者处理会议生命周期的不同阶段，可以协同工作——记录工具上一场会的产出，会喂进准备系统为下一场会准备的上下文里。

### 涉及机密会议时能用吗？

日历驱动的准备系统需要访问日历事件元数据（标题、参会人、时间）来触发准备，也需要邮件与文档源来收集上下文。对机密会议，准备材料在本地拼装——不会上传到云服务处理。系统在数据所在之处处理数据。如果本地处理是硬性要求，请确认工具支持设备端 AI，而不只是云端处理。

### 要让这套系统跑起来，我需要哪些集成？

最低要求是日历权限。要做完整的上下文收集，再加上邮件和至少一个文档存储集成（Google Drive、Notion、Dropbox 或本地文件系统）。任务管理器集成可补上待办事项结转。系统能拉取的数据源越多，准备材料越丰富——但哪怕只有日历权限，也能支撑基础准备（事件元数据、参会人识别、时间安排）。

### AI 需要访问我所有的邮件和文件吗？

它需要访问你想让它拉取上下文的数据源。多数系统允许你划定访问范围——只连特定邮件标签或文件夹、特定文档目录、特定日历。你无需把所有收过的邮件都开放给它。系统只使用它准备会议所需的内容。对多数用户来说，把范围限定在工作邮件与工作文档就够了。
