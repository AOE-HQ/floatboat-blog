...
### 单人经营者该不该自建 Lark CLI 集成？

多数情况下不建议作为首选。只有当集成是你业务的核心、你或队友经常写代码、现有工具覆盖不了、并且能持续投入维护时，才值得自建。如果只是锦上添花的工作流，或者坏了只有你一个人能修，就选更轻的路。真正的问题不是「我能不能搭出来」，而是「我没时间修它的那一周会发生什么」。

### Lark CLI 是什么？主要给谁用？

它是面向 Lark/飞书开放平台的命令行工具，覆盖消息、文档、多维表格、电子表格、日历、邮件、任务、会议等核心域——200+ 条命令和 19 个 AI Agent Skill。说白了：让你从终端或 AI Agent 以编程方式操控 Lark 工作区。它主要为把 Lark 集成进 bot、内部应用与自动化管线的开发者设计；如果你想要的是拖拽式集成，它大概不适合你。

### 用 Lark CLI 搭集成，实际要付出多少成本？

比看上去多得多。一个能稳定干点真事的基础集成，称职的开发者要花约一整天，非开发者至少翻倍。动手前要创建 Lark 应用（App ID 与 App Secret）、选对 token 类型、处理过期——用户 token 只有两小时有效；需要代表用户操作时还得配 OAuth 回调。部分 API 权限要管理员批准，之后还有按季度计的维护。

### 为什么 Lark CLI 集成会悄悄坏掉？

因为维护往往只有你一个人。Lark 更新 API，命令开始返回异常结果而没人盯着；token 刷新逻辑在假期周挂掉，自动化停了，直到客户来问才被发现；六个月后想交接或写文档，你已忘了半边配置的用途。token 过期是最常见的静默故障——不做自动刷新，就要做好每隔几小时无声失灵的准备。bus factor 是 1：就是你。

### 单人运营该用什么替代方案？

先试 Lark 自带的 webhook——简单、没有认证的复杂流程、容易测试。Zapier、Make、n8n 都有 Lark 集成：灵活度低一些，但维护负担在它们那边。如果你已经在用支持 MCP 的 AI 助手，lark-mcp 包是值得探索的中间路线——仍偏技术，但它是为 AI 辅助工作流设计的，而不是原始 API 脚本。

### Lark CLI 和 lark-mcp 是一回事吗？

相关但不同。Lark CLI（larksuite/cli）是给人和 Agent 用的命令行工具；lark-mcp 则把同一套 Lark API 封装成 Model Context Protocol 工具，让 AI 助手可以直接调用——文档处理、会话管理、日历排程都能做。如果你在用 Cursor 或 Claude 这类 AI 工具，lark-mcp 是更对路的选择。
