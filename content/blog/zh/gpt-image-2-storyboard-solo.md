...
能用。宽高比覆盖从 3:1 到 1:3，竖屏完全没问题。

### 认真做这件事需要 Plus 吗？

做分镜的话，基本需要。Thinking 模式与 8 帧批生成都在 Plus 及以上档位。免费档的 Instant 模式能画单帧，却会丢掉让这件事有价值的那份连续性优势。[DataCamp 的评测](<https://www.datacamp.com/blog/chatgpt-images-2-0>) 把档位划分讲得很清楚。

### API 每帧实际成本多少？

独立评测认为按分辨率与 prompt 复杂度，API 单图成本大约 0.04–0.35 美元。走 API 画一套 24 帧分镜，粗略估算是 5–8 美元。

### 分镜图能商用吗？

直接去查 OpenAI 现行使用政策——产出通常可商用，但涉及品牌与肖像权的细则会变。这不是法律建议，只是提醒你在发布之前查清楚。
