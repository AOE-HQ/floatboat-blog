...
### H3 Max Live 和 MiniMax H3 一样吗？

不一样。**MiniMax H3** 是基础全模态视频模型（H3-Base 开放权重、2K 路径、Ref2VA）。**H3 Max** 是 fal 在自家 API 上以 768p 提供服务的 post-trained、速度优化变体。**H3 Max Live** 是建立在 H3 Max 之上的**直播工作流**——外加一个实验性连续性端点——不是 MiniMax 的独立发布。

### 我今天就能用 fal 的公开 API 跑一个无限 AI 直播吗？

你可以用公开的 **H3 Max** 文生视频与图生视频端点、一个队列和 RTMP 工具自己搭出这个循环。H3 Max Live 展示的 fal **原生连续性**端点被描述为实验性的；在把跨场景记忆依赖到生产之前，先查 fal 的文档和模型目录确认 GA 状态。

### Twitch 或 Kick 为什么要标记 AI 直播？

平台通过自动化系统和举报执行社区准则。完全由 AI 生成、由聊天指挥、NSFW 风险不可预测的内容，并不能干净地套进现有创作者品类。2026 年 8 月报道的摩擦，可能反映的是审核规则、品类错配或观众举报——而不是技术上的直播能力不足。政策因平台而异，并会随 AI 直播的扩散而变化。

### 24/7 的 H3 Max AI 直播要多少钱？

按 fal 约 **768p 每秒 0.08 美元**的目录价，每个钟头生成一小时画面约需 **288 美元**；满打满算一天接近 **6,900 美元**，还不算失败任务、并行冗余或更低占空比。真实演示在赞助或间歇生成下引用约 **4,000 美元/天**。请按聊天峰值负载来预算，而不是按平均片段长度。

### 这和 MiniMax M3 或 Realtime API 有什么不同？

**MiniMax M3** 是面向 Agent 与编码的、以文本为中心的前沿 LLM。**MiniMax Realtime API** 面向低延迟的**语音**对话。**H3 Max Live** 是把**视频片段生成**接进广播缓冲。H3 Max 报道里出现的「realtime」，指的是**快过播放**，不是语音对语音对话。

### 什么时候用 H3 Max，什么时候用标准 MiniMax H3？

**需要 2K 输出、多资产的参考图转视频、或基于指令的剪辑时用标准 H3（fal 或 MiniMax 上都行）——画质与模态广度优先。需要最大吞吐量**、768p 下更强的提示词遵从性、或互动/高量生成（包括直播缓冲）时用 H3 Max。很多团队会两个都用：H3 Max 走速度路径，标准 H3 出主镜头。
