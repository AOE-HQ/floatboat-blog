...
对一次性原型、或你打算用完就扔的个人项目：vibe coding 就挺好，快、好玩、风险低。对任何你打算扩展、维护、或拿给用户看的东西：是的，结构化的 AI Agent 工作流会用更少的总返工换来更好的结果。vibe coding 大概能带你走到 70% 的位置——初稿看起来很棒，但功能越加越多、你越想去迭代时，应用就开始坏，最后还是得有人类收拾残局。结构补上的正是这段距离。

### 需要会写代码吗？

不一定，但你得能读懂输出、能发现看起来不对劲的东西。审查步骤需要的是判断力，不是语法知识。如果你无法判断输出是否做了你要的事，就抓不住那些会复利放大的失效模式。用哪个工具都一样。

### 应该从哪些工具开始？

从简单的开始。选一个 Agent 工具、养成一个版本控制习惯。每月 $20 的 Cursor 对不习惯终端的人是最容易的入口；当你在更大、更复杂的项目上工作时，Claude Code 值得升级。工具本身，不如围绕它的工作流纪律重要。
