...
### 为什么要构建自定义 Agent，而不是直接用 DeepSeek-TUI 或 Reasonix？

DeepSeek-TUI 和 Reasonix 是优秀的终端编码 Agent，但它们是专为开发者工作流造的。如果你的 Agent 需要做编辑代码以外的事——查询专有数据库、对接内部 API、自动化业务流程——自定义 Agent 循环让你对工具面有完全的控制。本教程的循环模式，是任何领域专用 Agent 的地基。如果你正在权衡"用 DeepSeek 写代码"与"留在 Claude Code"之间的成本与能力取舍，[DeepSeek Agent vs Claude Code](</blog/deepseek-agent-vs-claude-code>) 有详细对比。

### 我能把 DeepSeek 用在 LangChain 或 LangGraph 里吗？

能。两个框架都把 DeepSeek 当作即插即用的 OpenAI 兼容供应商来支持。在环境里设置 `OPENAI_API_BASE=https://api.deepseek.com` 和 `OPENAI_API_KEY=your-deepseek-key`，然后用标准的 `ChatOpenAI` 类、`model="deepseek-v4-pro"` 即可。本教程讲的 Agent 循环模式，正是 LangChain 的 `AgentExecutor` 在底层做的事——当 LangChain 的抽象坏掉时（它们确实会坏），理解原始循环就很有用。

### 如果模型调用了一个不存在的工具怎么办？

§6 的修复模式就是干这个的。返回一个结构化错误——`{"error": "Unknown tool: X. Available: [list]"}`——模型通常会在下一回合自我纠正。V4 模型的自我纠正能力显著强于 V3，但你仍应在执行前校验每一次工具调用。
