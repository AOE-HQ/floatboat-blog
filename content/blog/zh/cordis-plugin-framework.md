...
不是。Cordis 是开发者 shigma 创建的独立开源元框架，作为 Koishi 聊天机器人框架的底座已有四年。DeepSeek Harness 构建在 Cordis 之上，所以它会出现在 DeepSeek 的架构文档里。

### 什么是元框架？

用来构建框架的框架。Cordis 不针对某个具体领域（比如聊天机器人或 Web 服务器）提供 API，而是提供插件系统、依赖管理与资源生命周期控制所需的基础机制。它的名字来自拉丁语 _cor_，意为"心脏"。

### "时空可组合性"是什么意思？

这是论文对动态系统所需的两个性质的命名：时间可组合性（组件的副作用在移除时可被彻底还原）与空间可组合性（组件可以声明依赖，由运行时响应式管理）。两者合在一起，让插件系统可以被安全地重载与演化。

### Cordis 如何清理插件的副作用？

通过 ctx.effect。每个会产生副作用的调用都返回一个清理函数，运行时把它存下来，在插件卸载时按注册的逆序执行。这相当于 C++ 的 RAII 或 Rust 的 Drop，但在运行时层面处理，所以第三方插件也能干净卸载。
...
一个能修改自身运行时的 Agent——加工具、改主循环、演化工作流——只有在其每一项变更都能被回滚时，才值得被信任。可逆副作用就是把"自修改 Agent"变成工程范式而非研究风险的那个机制。
