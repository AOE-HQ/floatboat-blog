...
有件事我给它起了个名字叫「重新交代税」——每次会话都要向 AI 重新解释自己，那种看不见的时间成本。单看一次会话不大，但会累积——尤其当你在做积累了大量上下文的长期项目时。它和[情境切换](/zh/blog/stop-context-switching-workspace-agent)损耗独立创业者的方式是近亲——只不过这里的切换不是日程逼的，是 AI 的失忆逼的。
...
能问出「上个月关于这件事我们定了什么」并真的拿到有用答案——这是我反复回到的用例。这类工作也正好在[真实场景里站得住的 agent 用例](/zh/blog/ai-agent-use-cases-real-examples)那份短名单上。
...
**如果你是单人创业者、内容创作者或知识工作者**、不碰基础设施——这个概念本身跟你有关系，但当前大多数持久化 Agent 的实现，还不是为你做的。这个空档正是[一人公司需要的工作区 agent](/zh/blog/workspace-agents-for-solo-operators) 在补的：持久性通过你本来就会打开的工作区交付，而不是一台要你维护的服务器。

搭建开销是实打实的。你得习惯在某台服务器上跑东西、管理配置、并容忍早期开源项目特有的那些毛糙边缘——这跟判断[定制 AI Agent 开发](/zh/blog/custom-ai-agent-development)到底值不值得付费，本质上是同一笔总拥有成本账。
