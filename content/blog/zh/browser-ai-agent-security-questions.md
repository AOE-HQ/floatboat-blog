...
从技术上，权限宽泛的扩展能读取页面内容，包括登录表单——所以 Anthropic 明确建议把密码管理器网站放进黑名单、排除在权限之外。操作确认步骤会增加静默窃取凭据的难度，但在提示词注入研究如此活跃的当下并非不可能。别拿真实凭据去测试，把密码管理器永久拉黑。

### 安装提示里那些宽泛权限，到底意味着什么？

「读取并更改你在所访问网站上的所有数据」通常意味着扩展用上了 debugger API 或宽泛主机权限——它能像开发者工具一样挂到标签页、深层读取页面状态，并直接在你已登录的会话里行动。这对替你点击、浏览的 Agent 是必要的，但它也是很大的信任面，绝非随手就能授予的小权限。

### 遇到新网站，我该选「仅允许一次」还是「始终允许」？

把「仅允许一次」当默认：偶尔用或正在测试的网站都选它。「始终允许」只留给 CRM 这类你高频使用且信任的工具。凡是点错一个按钮要花 30 分钟以上才能补救的网站一律拉黑——银行、支付账户、密码管理器。别开「始终允许浏览器内容」，也要一直保持「行动前询问」模式。

### 提示词注入的风险有多真实？

这是已被演示、可量化的真实风险，不是纸上谈兵。Anthropic 报告：Claude for Chrome 无防护时攻击成功率约 23.6%，启用现有防御后仍约 11.2%——大约每 9 次就有 1 次得手。所以要把页面当不可信输入：握着敏感账号别上论坛、评论区，一跑题就停。

### 关掉记忆功能，我的数据就完全安全了吗？

不是——关记忆只是限制跨会话复用，并不等于完全隔离。按 Codex 的文档，浏览器活动一旦进入 Codex 上下文仍会被存储；Claude 的记忆开关同样不保证服务端不存任何东西。数据控制范围覆盖多少，取决于你的账户档位。真正敏感的任务，用全新会话并在任务之间清空上下文。

### 什么情况下我完全不该用浏览器 AI Agent？

当「做错一步的后果不可逆或受监管」时：计费变更、发送邮件、删除数据、会触发下游流程的表单提交——把真人留在回路里。受监管行业、受 NDA 保护的客户数据、共享或管理员账号也一律别接，任何错误的爆炸半径都会被放大。不清楚自己的合规义务，先问法务。
