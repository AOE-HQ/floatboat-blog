...
不便宜。Grok 3 收 $3/百万输入、$15/百万输出，定价紧挨 Claude Sonnet，属高端档而非平价。想省钱该用 Grok 3 Mini（$0.30/$0.50）或 Grok 4.1 Fast（$0.20/$0.50）。所以「Grok 变便宜了」，说的是 Mini 和 Fast，不是 Grok 3。

### Grok 3 的「开源」到底指什么？

两层含义要分开。API 访问开放没错；但权重开源另说——写作时 Grok 3 权重未上 Hugging Face，Grok 2.5 也是带反竞争条款的 Community License，并非 MIT 或 Apache。若延续同样模式，「开放权重」不等于「可自由商用」。

### Grok 3 API 可以免费试用吗？

新账号给 25 美元额度，无需预先绑卡，够你跑几轮真实测试再决定付费。条款会变，以 xAI 官网当前优惠为准。月花费不到约 30 美元的单人用户，试用额度基本就够完成一次有意义的评估。

### 什么时候切换更便宜的 API 才真正划算？

同一套工作流已跑了好几个月、账单爬过约 80 美元/月、且你清楚哪些 prompt 形状有效时，切换才是真杠杆。月花费低于约 30 美元，切换只省个位数美元，还要赔上一天重测 prompt。如果主要摩擦是把步骤接起来，模型单价不是该先解决的问题。

### 便宜的 token 不是真正的门槛，那是什么？

是工作流本身——把步骤接起来花掉的时间：拉文档、交给模型、粘贴输出、二遍润色、存到找得到的地方，这条链吃掉整个下午；调 Grok 3、GPT 还是 DeepSeek，只是链上的一步。托管式 Agent 平台管的是整条链；裸 API 单次更便宜，但管道工得自己做。先想清楚稳定工作流，再谈优化价格。

### 单人工作里，Grok 3 比 Claude 或 GPT 更好吗？

对多数单人工作流没有明显差别。按适配度选：哪个更配你的写作风格，以及你是否需要实时 X 数据——Grok 的 X 集成确实独一份。要高吞吐低成本输出，Grok 3 Mini 或 DeepSeek 比任何高端模型都合理。适配度与工作流经济学，比基准分数重要。
