...
新账号会拿到 25 美元额度，无需预先绑卡。条款会变，[直接核对当前优惠](<https://x.ai/api>)为准。

### 权重发布后，我该自托管 Grok 3 吗？

如果你是单干，大概不该。这个模型约有 500GB。除非你有正经的 GPU 配置或真实的合规理由，否则 API 访问比电费便宜。

### 单人工作里，Grok 3 比 Claude 或 GPT-5 更好吗？

老实说，对多数单人工作流——没有明显差别。按哪个模型配你的写作风格、以及你是否需要实时 X 数据来选。适配度比基准分数重要。
