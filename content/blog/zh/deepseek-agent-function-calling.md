...
本文假设你已理解基本 Agent 循环——发消息、查工具调用、执行、回传、重复。如果这套模式对你陌生，先读 如何构建 DeepSeek Agent。我们这里聚焦工具调用层内部发生的事：schema 设计、并行执行、严格模式、与思考模式的交互，以及那些防止 Agent 循环在畸形参数上崩溃的生产级模式。
...
如何构建 DeepSeek Agent 里的最小循环，每轮只处理一个工具调用。生产级 Agent 还需要三个额外控制：`tool_choice`、并行调用处理、会话状态保全。
...
完整的 Agent 架构——API 配置、模型选择、以及本文所扩展的那个循环骨架——如何构建 DeepSeek Agent 每一步都带可运行代码走了一遍。
