...
### DeepSeek V4 能同时支持 JSON 模式和函数调用吗？

可以。你可以在最终答案上设 `response_format: {"type": "json_object"}`，同时中间步骤照常用 `tools`。常见套路：用工具调用收集数据，最后一轮设 `tool_choice: "none"` 并开 JSON 模式输出结构化结果。但别在预期出现工具调用的轮次开 JSON 模式——模型可能优先保证 JSON 格式，而忽略工具选择。

### 函数调用和 MCP 有什么区别？

函数调用在 API 请求里内联定义工具——由你的代码在本地执行。MCP 把工具定义在外部服务器上，由你的 Agent 在运行时发现并调用。函数调用更简单、上手更快；当工具是动态的、数量多、或由不同团队维护时，MCP 扩展性更好。两者在 Agent 循环里产出的是同一种消息格式。

### 相比 GPT-4o 或 Claude，V4 的工具调用有多可靠？

在 MCPAtlas Public 上，V4 Pro 得了 73.6——与 Claude Opus 4.6 在 agentic 工具使用基准上打平。实践中，可靠性更多取决于你的 schema 设计与修复层，而不是模型本身。一套设计良好、带结构化错误反馈的工具 schema，在任意顶级模型上都能跑出高成功率；一份含糊的 schema，则会在所有模型上翻车。

### 我能在 Anthropic 兼容端点上用函数调用吗？

可以。DeepSeek 同时开放 OpenAI 兼容端点（`https://api.deepseek.com`）与 Anthropic 兼容端点（`https://api.deepseek.com/anthropic`）。走 Anthropic 端点的工具调用用的是 Anthropic 的工具格式，而不是 OpenAI 的 `tools` 数组。如果你在配置 Claude Code 把 DeepSeek 当后端，harness 会替你处理这个转换——工具定义不用你自己写。

### 工具密集的 Agent 循环该用 V4 Pro 还是 V4 Flash？

工具执行轮默认用 V4 Flash——一个任务几十次调用，速度和成本优势是复利式的。规划轮（决定调哪些工具）和综合轮（把结果整合成最终答案）用 V4 Pro。按这种分工跑一个典型的 15 轮 Agent，成本大约比全程 V4 Pro 低 60–70%，多数任务质量损失极小。
