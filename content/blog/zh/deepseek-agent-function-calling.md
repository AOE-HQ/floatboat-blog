...
V4 Pro 与 V4 Flash 都原生支持，请求里带上 tools 数组即可。API 兼容 OpenAI 格式，与 GPT-4o、Claude 的用法一致。两款模型单轮最多支持 128 个并行工具调用。想控制成本，工具执行轮多用 Flash，规划和综合轮交给 Pro。

### 工具调用循环到底怎么跑？

模型判断某工具有用时，不直接回答，而是返回结构化工具调用。你的代码执行后，把结果以 tool 消息配上对应调用 id 追加进历史，再发回 API 继续，直到模型给出最终答案。最容易踩坑：回传时原样保留 assistant 消息里的 tool_calls，否则结果与调用会失联。

### 思考模式和函数调用能一起用吗？

可以。V4 能把思考与工具调用组合，这是早几代模型做不到的。开启后模型会先内部推理再发工具调用；循环跑通并不依赖这段推理，但记日志有助调试。成本上，思考 token 按输出计费，高强度推理一次可能多花几百 token，建议只开在规划轮与综合轮。

### DeepSeek V4 支持并行工具调用吗？什么时候该避免？

支持。V4 单轮最多返回 128 个工具调用；逐个独立执行，每个结果作为独立 tool 消息追加即可，id 对上、顺序无所谓。并行省延迟不省 token——结果都计入上下文。工具碰共享状态时别并行（如读写同一路径会产生竞态）。建议先串行跑稳，再给搜索这类只读工具开并行。

### 内联函数调用和 MCP 有什么区别？

内联函数调用把工具定义写进请求，由你的代码本地执行。MCP 把工具放到外部服务器，由 Agent 在运行时发现并调用，适合工具动态变化或由其他团队维护。两种方式在循环里消息格式相同，只是执行层不同。先用内联，等工具超过约 20 个、频繁变动或需要沙箱这类隔离环境时，再切到 MCP。

### 生产环境里 Agent 循环为什么坏？怎么修？

生产故障大多出在工具调用层而非模型：JSON 畸形、工具名写错、参数类型不对。先用清晰的 schema 预防，参数至关重要时开严格模式；再建修复层：先校验后执行，把结构化 JSON 错误返回给模型让它自我纠正。还要设轮次上限、连错即停、记录每次调用，并截断大段工具输出，防止上下文膨胀。
