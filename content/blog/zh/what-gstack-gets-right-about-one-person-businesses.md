...
不需要。这篇文章里的一切在 Claude.ai、ChatGPT 或任何通用聊天界面里都成立——你改变的只是开场会话的方式，不需要安装工具。gstack 本身是给开发者用的 Claude Code 技能包，但底层的角色化模式可以转成一套非代码工作流：先说出认知模式、把阶段拆进不同会话、把摘要存档往前带。

### 这不就是写个更好的提示词吗？

它是提示词里的一个特定模式。关键区别在于：你显式约束任务所需的认知框架——定义该优先什么、该忽略什么、什么算成功——而不只是加细节。而且「不该做什么」的约束，往往比你要求它做什么更有价值。

### 如果我不知道该用哪个角色怎么办？

这说明你处在前置阶段：先把问题本身搞清楚。那部分自己做，或显式跑一次自由探索会话——告诉模型你还没有明确的问题，想一起想想。对混沌期的问题过早强加角色，反而会收窄思路；等问题清晰了，再进入角色化阶段。

### 这对创意工作也适用吗？

适用——只是角色的样子不同。「发散式创意模式——先出十个方向、不做筛选」和「编辑模式——收紧、删减」不是一回事，但原理相同。要紧的不是角色标签：关于角色提示的研究结论并不一致，收益来自你绑在角色上的约束与范围。

### 这和每个任务用独立工具有什么不同？

更轻。你是在一个工具内切换框架，而不是切换应用——拿到专业化的大部分好处，却不必付上下文切换的代价。系统提示词设定一般行为；角色化框架更进一步，把「这一步该优化什么」圈出来，更像有位专注的专家，而不是在一堆应用间来回倒腾。

### 我设的角色能跨会话延续吗？

默认不能。gstack 的虚拟团队没有共享上下文——CEO 角色周二的洞见，周三就没了；同时跑多条工作流时，这是真实的摩擦。对策是手动的：把 3–5 句摘要存进下一个会话，或用一层替你保存上下文的工作区载体——Floatboat 的 Tacit Engine 正是这个方向，它随时间学习你的工作模式。
