...
import { parseHeadingAnchor, resolveDisplayImageUrl } from "@openblog/core";
  ...
  const components = useMemo(
    () => ({
      h2: ({ children, ...props }: React.ComponentPropsWithoutRef<"h2">) => {
        const { title, id } = parseHeadingAnchor(String(children));
        return (
          <h2
            id={id}
            className={cn(
      ...
      img: ({
        ...
        const resolved = resolveDisplayImageUrl(typeof src === "string" ? src : undefined);
        if (!resolved) {
          return null;
        }

        return (
          // Use block-level img only — <figure> inside markdown <p> breaks HTML nesting and hydration.
          // eslint-disable-next-line @next/next/no-img-element
          <img
            src={resolved}
            alt={alt ?? ""}
            className={cn(
              "my-8 block w-full rounded-xl border object-cover",
              obBorder,
            )}
            loading="lazy"
            {...props}
      ...
      table: ({ children, ...props }: React.ComponentPropsWithoutRef<"table">) => (
        <div className="my-6 overflow-x-auto">
          <table
            className={cn("w-full min-w-[32rem] border-collapse text-sm", obText)}
            {...props}
          >
            {children}
          </table>
      ...
      thead: ({ children, ...props }: React.ComponentPropsWithoutRef<"thead">) => (
        <thead className="bg-[var(--ob-color-surface)]" {...props}>
          {children}
        </thead>
      ),
      ...
      td: ({ children, ...props }: React.ComponentPropsWithoutRef<"td">) => (
        <td className={cn("border px-3 py-2 align-top leading-6", obBorder, obText)} {...props}>
          {children}
        </td>
      ),
    }),
