...
export interface OpenBlogOptionalComponents {
  ...
  prevNext?: boolean;
  /** FAQ accordion parsed from markdown */
  faq?: boolean;
  /** Final CTA block parsed from markdown / config */
  finalCta?: boolean;
}
...
export interface OpenBlogBlogCtaCopy {
  title: string;
  description?: string;
...
export interface OpenBlogBlogConfig {
  cta?: {
    fallback?: {
      en?: OpenBlogBlogCtaCopy;
      zh?: OpenBlogBlogCtaCopy;
    };
    slugs?: Record<string, { en?: OpenBlogBlogCtaCopy; zh?: OpenBlogBlogCtaCopy }>;
  };
}
...
export interface OpenBlogConfig {
  ...
  /** Blog-specific extensions (FAQ/CTA config, etc.) */
  blog?: OpenBlogBlogConfig;
}
...
export function resolveOptionalComponents(
  ...
  return {
    ...
    faq: opt.faq ?? DEFAULT_OPTIONAL_COMPONENTS.faq,
    finalCta: opt.finalCta ?? DEFAULT_OPTIONAL_COMPONENTS.finalCta,
  };
}
